...
RobustVariance<-function(stepsize,n, thetahat, LLH,cluster_ID) {
  ...
  # grid over all entries of thetahat
  for (n1 in 1:length(thetahat)) {
    for (n2 in 1:length(thetahat)) {
      ...
      # approximates the gradient by looking at change in LLH at this value of theta
      # 2 * stepsize is the difference in theta values between the two LLHs
      # there are two sets of difference terms because it's a second derivative (difference in differences)
